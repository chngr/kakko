% gen_opr_index(): general funciton  E_p and F_p
...
function [result_indices, num_tuple, result_mat] = gen_opr_index(index,opr,k,basis)
W = basis{index};
r = size(W,1);
...
for j = 1:length(k_tuples)
    ...
    if flag == true
        ...
        e = new_mat(diff(1),diff(2));
        if opr == 'E'
            num_tuple{end+1} = [0,a,b,c,d,e];
        else
            num_tuple{end+1} = [1,a,b,c,d,e];
        end
        ...
        new_trans_mat(r-1,r)= e;
        new_trans_mat(r,r-1)= e;
        
        % Add new binary and matric to cell arrays
        result_mat = [result_mat,new_trans_mat];
        [~,trans_index] = mat_2_tuple(new_trans_mat);
        result_indices = [result_indices, trans_index];
    end
end
end