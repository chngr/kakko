% bracket_operation: infinitely compute bracket of E and F
% input: two matrices E and F representing operation on basis

function [dim,result_basis] = bracket_operation(p)
[map, ~] = gen_basis(p);
grouping = grouping_basis(p);
[E,F] = grouping_2_mat(grouping,p,map);
vector_list = cell(0);
...
while true
    ...
    if isempty(temp_list)
        ...
        result_basis = [old_list,new_list];
        fprintf('The whole basis')
        disp(result_basis);
        dim = dimension;
        break;
    end
