...
# This file was *autogenerated* from the file /Users/alejandrobuendia/Desktop/kakko/RamseyNumber/classification/mat_to_txt.sage
from sage.all_cmdline import *   # import sage library

_sage_const_2 = Integer(2); _sage_const_1 = Integer(1); _sage_const_0 = Integer(0)


# REGULAR PRINTING (THREE-LEVEL)
    ...
    for index in range(len(mat_set)):
        ...
        for i in range(len(cur_block)):
            ...
            for j in range(cur_mat.nrows()):
                ...
                for k in range(len(cur_row)):
                    f.write(str(cur_row[k]))
                    if k != len(cur_row) - _sage_const_1 :
                        f.write(',')
                f.write(']')
                if j != cur_mat.nrows() - _sage_const_1 :
                    f.write(',')
            f.write(']')
            if i != len(cur_block) - _sage_const_1 :
                f.write(',')
        f.write('];')
        f.write('\n\n')
...
def bracket_operation(gen_mat,gen_names):
    ...
    while True:
        ...
        for i in range(len(new_list)):
            for j in range(i+_sage_const_1 ,len(new_list)):
        ...
        if len(temp_list) == _sage_const_0 :
            dim = len(old_list)
...
def in_span(in_list, entry):
    col_len = (in_list[_sage_const_0 ].ncols())**_sage_const_2 
    comp_mat = matrix(QQ,col_len,_sage_const_0 )
...
in_file_name = argv[_sage_const_1 ]
# get out_file name
cur_dir = getcwd()
out_file_name = 'gap_' + argv[_sage_const_1 ]
out_full_name = path.join(cur_dir,'..','gap_files',out_file_name)
...
for i in range(_sage_const_0 ,len(block_list)):
    gen_mat = []
    gen_names = []
    # gen_mat and gen_names for bracket_operation
    gen_mat.append(matrix(QQ,block_list[i][_sage_const_0 ]))
    gen_mat.append(matrix(QQ,block_list[i][_sage_const_1 ]))
    ...
    print('\n')
# print to output
mat_to_txt(basis_set,out_full_name)