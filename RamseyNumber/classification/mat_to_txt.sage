...
def bracket_operation(gen_mat,gen_names):
    ...
    for name in gen_names:
        ...
        f_count = name.count("f")
        grade = e_count - f_count
        if grade not in freq_map:
            freq_map[grade] = 1
        else:
            freq_map[grade] += 1
        print('grade: %d' % grade)
    ...
    while True:
        ...
        for i in range(len(new_list)):
            for j in range(i+1,len(new_list)):
                ...
                if not in_span(old_list + new_list + temp_list, new_entry):
                    ...
                    # store freq in dict
                    e_count = name_new_entry.count("e")
                    f_count = name_new_entry.count("f")
                    grade = e_count - f_count
                    if grade not in freq_map:
                        freq_map[grade] = 1
                    else:
        ...
        print('current dimension: %d' % len(old_list + new_list))
        print('current frequencies: %s' % freq_map)
...
for i in range(0,len(block_list)):
    ...
    print('\n')
# print to output
mat_to_txt(basis_set,out_file_name)